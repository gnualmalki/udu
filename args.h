...
static const char *USAGE =
  ...
  "  -X, --exclude=PATTERN  skip files or directories that match a glob "
  "pattern\n"
  "                          *        any characters\n"
  "                          ?        a single character\n"
...
UDU_SI bool ensure_capacity(char ***array, int *capacity, int count)
{
    if (count >= *capacity)
    {
...
UDU_SI void args_init(args_t *args)
{
    memset(args, 0, sizeof(args_t));
}

UDU_SI void args_free(args_t *args)
{
    free(args->paths);
    free(args->excludes);
    memset(args, 0, sizeof(args_t));
...
UDU_SI void args_print_help(void)
{
    printf("%s", USAGE);
}

UDU_SI void args_print_version(void)
{
    printf("UDU %s\n%s", VERSION, LICENSE);
}

UDU_SI bool handle_short_option(char opt,
                                args_t *args,
                                int *i,
                                int argc,
                                char **argv,
                                int *exclude_capacity)
{
    switch (opt)
    {
        ...
        case 'v':
            args->verbose = true;
            args->quiet = false;
            // args->tree = false; (if true goes in tree-verbose mode)
            return true;
        case 'q':
            args->quiet = true;
            args->verbose = false;
        ...
        case 't':
            args->tree = true;
            args->quiet = true;
            // args->verbose = false; (if true goes in tree-verbose mode)
            return true;
        case 'X':
            if (*i + 1 >= argc)
            {
...
UDU_SI bool args_parse(args_t *args, int argc, char **argv)
{
