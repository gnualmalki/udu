...
static inline bool glob_match_impl(const unsigned char *pat, const unsigned char *txt)
{
    while (*pat)
    {
        if (*pat == '*')
...
UDU_SI bool glob_match(const char *pattern, const char *text)
{
    return pattern && text && glob_match_impl(UC(pattern), UC(text));
}

UDU_SI char *path_join(const char *parent, const char *child)
{
...
UDU_SI const char *path_basename(const char *path)
{
    if (!path || !*path) return "";
...
UDU_SI char *human_size(uint64_t bytes, char *buf, size_t buflen)
{
